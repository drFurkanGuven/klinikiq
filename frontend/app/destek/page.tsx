    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground overflow-y-auto">
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 py-12">
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-black text-foreground">Destek</h1>
        ...
        <div className="bg-surface-muted rounded-2xl border border-border p-6 mb-6">
          ...
          <p className="text-muted text-sm leading-relaxed mb-4">
            ...
            yazabilirsiniz. Genelde <span className="text-foreground font-bold">24-48 saat</span>{' '}
          ...
          <div className="mt-4 space-y-2">
            ...
            <ul className="text-sm text-muted space-y-1.5 pl-3">
          ...
          <div className="mt-4 flex items-center gap-2 text-xs text-muted">
            ...
            <span>
              Hafta içi <span className="text-foreground font-bold">09:00 — 22:00</span>{' '}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {CATEGORIES.map((c) => {
              ...
              return (
                <div
                  key={c.title}
                  className="flex gap-3 p-3 rounded-xl border border-border bg-surface-muted"
                >
                  <div
        ...
        <div className="bg-surface-muted rounded-2xl border border-border p-6 mb-6">
          ...
          <div className="space-y-3">
            {FAQ_ITEMS.map((item) => (
              <details
                key={item.q}
                className="group rounded-xl border border-border bg-surface-muted overflow-hidden"
              >
                <summary className="flex items-center justify-between gap-3 cursor-pointer list-none [&::-webkit-details-marker]:hidden p-4 text-sm font-bold text-foreground hover:bg-surface-muted transition-colors">
                  <span className="flex-1">{item.q}</span>
                  <span className="text-muted text-xs group-open:rotate-180 transition-transform shrink-0">
                    ▼
                  </span>
                </summary>
                <div className="px-4 pb-4 -mt-1 text-sm text-muted leading-relaxed">
                  {item.a}
                </div>
              </details>
            ))}
        ...
        {/* Tıbbi Uyarı */}
        <div className="bg-surface-muted rounded-2xl border border-border-strong p-6 mb-6">
          <h2 className="text-lg font-black text-foreground mb-3 flex items-center gap-2">
            <AlertTriangle className="w-5 h-5 text-warning" />
            Önemli Tıbbi Uyarı
          </h2>
          <p className="text-sm text-foreground leading-relaxed">
            KlinikIQ tamamen <span className="font-bold text-foreground">eğitim amaçlıdır</span>{' '}
          ...
          <p className="text-xs text-muted mt-3">
            Acil tıbbi durumlarda <span className="text-foreground font-bold">112</span>{' '}
            (Türkiye) numarasını arayın.
          </p>
        </div>

        {/* Yararlı Linkler */}
        <div className="bg-surface-muted rounded-2xl border border-border p-6">
          <h2 className="text-lg font-black text-foreground mb-4">Yararlı Bağlantılar</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Link
              href="/privacy"
              className="flex items-center gap-3 p-3 rounded-xl border border-border bg-surface-muted hover:bg-surface-muted transition-colors"
            >
              <ShieldCheck className="w-4 h-4 text-purple-400 shrink-0" />
              <span className="text-sm font-bold text-foreground">Gizlilik Politikası</span>
            </Link>
            <Link
              href="/terms"
              className="flex items-center gap-3 p-3 rounded-xl border border-border bg-surface-muted hover:bg-surface-muted transition-colors"
            >
              ...
              <span className="text-sm font-bold text-foreground">Kullanım Şartları</span>
            </Link>
            <Link
              href="/delete-account"
              className="flex items-center gap-3 p-3 rounded-xl border border-border bg-surface-muted hover:bg-surface-muted transition-colors"
            >
              ...
              <span className="text-sm font-bold text-foreground">Hesap Silme</span>
            </Link>
            <a
