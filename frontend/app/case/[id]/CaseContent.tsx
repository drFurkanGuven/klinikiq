...
export default function CasePageContent() {
  ...
  async function handleComplete() {
    ...
    try { await sessionsApi.complete(sessionId); router.push(`/report/${sessionId}`); }
      ...
      <nav className="glass border-b flex-shrink-0 transition-all font-sans" style={{ borderColor: "var(--border)", background: "var(--surface)" }}>
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="md:hidden p-1.5 rounded-lg transition-colors shadow-sm" style={{ background: "var(--surface-2)", color: "var(--text-muted)" }}>
              {sidebarOpen ? <PanelLeftClose className="w-4 h-4" /> : <PanelLeftOpen className="w-4 h-4" />}
            </button>
          </div>
          <div className="flex items-center gap-2 sm:gap-4">
            <ThemeToggle />
            <button onClick={() => setLabOpen(true)} className="group flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md border relative overflow-hidden" style={{ background: "var(--primary)", borderColor: "var(--primary-h)", color: "white" }}>
               <div className="absolute inset-0 bg-white/10 w-0 group-hover:w-full transition-all duration-300" />
               <TestTube2 className="w-3.5 h-3.5 relative z-10" /> <span className="relative z-10">Tetkik İste</span>
            </button>
            <button onClick={() => setExamOpen(true)} className="group flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-md border relative overflow-hidden" style={{ background: "var(--surface-2)", borderColor: "var(--border)", color: "var(--text-navy)" }}>
               <div className="absolute inset-0 bg-black/5 w-0 group-hover:w-full transition-all duration-300" />
               <Stethoscope className="w-3.5 h-3.5 relative z-10" style={{ color: "var(--primary)" }} /> <span className="relative z-10">Fizik Muayene</span>
            </button>
